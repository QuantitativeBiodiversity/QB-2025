...
Now that you are logged into Posit.cloud, you can go to the page called "Your Workspace", which should currently be empty. In the right hand corner, there is a button called "New Project." Click and select the dropdown "New RStudio Project." A new project should appear with RStudio as an isolated environment. Note that any modification that you make (e.g., file created or package installed) will be saved within this project and will not be lost when you exit. However, the contents are *only accessible when the project itself is open* in Posit.Cloud. This means that if you create additional New RStudio Projects, each one will be a clean slate.

Once RStudio in Posit.cloud has loaded, open the script editor from the File menu on the top left corner:
...
For your convenience you might also want to clear the console. this can be done usin `CTRL` + `L`. 
