...
On your classroom computer, you will be able to access a **cloud version** of RStudio (https://posit.cloud). 
Not only is the cloud version of RStudio accessible anywhere, it also comes with Git pre-installed. 
...
Finally, you will learn how to **commit** and **push** changes so that you can access them later using **fetch** and **merge** commands.
In QB, you will submit work to instructors for evalution by making a **pull request**.
...
3. After successfully joining, you should see in the left navigation bar, under **Spaces*, both *Your Workspace* and *QB-2025*
...
When using a free Posit.cloud account (e.g.,: *Your Workspace*), memory is limited to 1 GB RAM. 
...
Last, because our Posit Cloud subscription is proportional to usage, we ask that you only use the QB-2025 Workspace for class-related work. 
...
2. The default setting of all new projects is "Private." 
Please keep projects private so **only you have editing capabilities**. 
Assignments will still be published publicly via Git, so this is entirely to protect students from accidentally opening and changing the wrong assignment unknowingly, since Posit.Cloud autosaves! 

3. While Git comes pre-installed with Posit Cloud, we are going to make a few changes to the default configuration:
...
4. Clone the repo into your empty RStudio project (named "Github") using the command line (Terminal).
Replace "username" with your GitHub username and "repo" with the name of the repo in the QBstudent organization (e.g., QB2025_Smith)
...
6. Using RStudio, open and edit the README.md file contained in your student repository:
    ...
    After the text "Quantitative Biodiveristy course, Spring 2025, Indiana University", enter your name and email address.
