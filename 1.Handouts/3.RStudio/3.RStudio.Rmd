...
For example, `"/cloud/project/QB-2025/Week-1"`, tells you that the folder `Week-1` is inside the folder `QB-2025`, which is inside the folder `project`, which is inside the folder `cloud`. 
...
To so, you can use the function `setwd()`
When you set your working directory, you are telling RStudio to think of this directory as the **root** directory. 
...
Since you have set the working directory (above) to something that includes the `Week-1` folder, e.g.,: `#setwd("/cloud/project/QB-2025/Week-1")`, any additional files you want to use will rely on paths that are based on that that working directory, which means you only need to include folders that come after `Week-1`.
